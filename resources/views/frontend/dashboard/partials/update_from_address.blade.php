<div class="modal-dialog modal-lg vertical-align-center" style="margin-top: 80px;">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title">Update Shipping Origin Address</h4>
            <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <div class="modal-body ">
            <div class="row">
                <div class="inter-form signUpForm">
                    <div class="row mt-4">
                        <label class="col-sm-3">Address</label>
                        <input  class="col-sm-9" type="text" name="csr_address1">
                    </div>
                    <div class="row mt-4">
                        <label class="col-sm-3">Apartment / Suite / Unit / Building etc</label>
                        <input  class="col-sm-9" type="text" name="csr_address2">
                    </div>
                    <div class="row mt-4">
                        <label class="col-sm-3">Department, C/D etc</label>
                        <input  class="col-sm-9" type="text" name="csr_address3">
                    </div>
                    <div class="row mt-4">
                        <label class="col-sm-3">Postcode</label>
                        <input  class="col-sm-9" type="number" name="csr_pincode">
                    </div>
                    <div class="row mt-4">
                        <label class="col-sm-3">City</label>
                        <input  class="col-sm-9" type="text" name="csr_city_id">
                    </div>
                    <div class="row mt-4">
                        <label class="col-sm-3">State</label>
                        <input  class="col-sm-9" type="text" name="csr_state_id">
                    </div>
                    <div class="row mt-4">
                        <label class="col-sm-3">Land Mark</label>
                        <input  class="col-sm-9" type="text" name="S_other">
                    </div>
                </div>
            </div>
        </div>
        <div class="modal-footer">
            <button type="button" data-dismiss="modal" class="btn btn-default">Close</button>
            <button type="button" data-dismiss="modal" id="udt_frm_add" class="btn btn-info">Update</button>
        </div>
    </div>
</div>