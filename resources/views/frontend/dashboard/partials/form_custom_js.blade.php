...
<script>
    ...
    function getCourierRates() {
        ...
        $('#actual_weight').val(pcs_weight);
        var formData = {
            package_type: packet_type,
            from_country: csr_country_id,
            to_country: csn_country_id,
            weight: pcs_weight
        };
        $.ajax({
            type: 'post',
    ...
    $('#courier_mc_1 option:selected').html($('#courier_mc_1 option:selected').attr('value')); // already changed onload

    $('#courier_mc_1').on('change', function() {
        $('#courier_mc_1 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
        $('#courier_mc_1 option:selected').html($('#courier_mc_1 option:selected').attr('value'));


        $(this).blur();
    });
    $('#courier_mc_1').on('focus', function() {
        $('#courier_mc_1 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
    });
    ...
    $('#courier_mc_2 option:selected').html($('#courier_mc_2 option:selected').attr('value')); // already changed onload

    $('#courier_mc_2').on('change ', function() {
        $('#courier_mc_2 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
        $('#courier_mc_2 option:selected').html($('#courier_mc_2 option:selected').attr('value'));
        $(this).blur();
    });
    $('#courier_mc_2').on('focus', function() {
        $('#courier_mc_2 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
    });

    $('#courier_mc_3 option:selected').html($('#courier_mc_3 option:selected').attr('value')); // already changed onload

    $('#courier_mc_3').on('change ', function() {
        $('#courier_mc_3 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
        $('#courier_mc_3 option:selected').html($('#courier_mc_3 option:selected').attr('value'));
        $(this).blur();
    });
    $('#courier_mc_3').on('focus', function() {
        $('#courier_mc_3 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
    });

    $('#courier_mc_4 option:selected').html($('#courier_mc_4 option:selected').attr('value')); // already changed onload

    $('#courier_mc_4').on('change ', function() {
        $('#courier_mc_4 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
        $('#courier_mc_4 option:selected').html($('#courier_mc_4 option:selected').attr('value'));
        $(this).blur();
    });
    $('#courier_mc_4').on('focus', function() {
        $('#courier_mc_4 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
    });
    ...
    $('#common_mc_1 option:selected').html($('#common_mc_1 option:selected').attr('value')); // already changed onload

    $('#common_mc_1').on('change ', function() {
        $('#common_mc_1 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
        $('#common_mc_1 option:selected').html($('#common_mc_1 option:selected').attr('value'));
        $(this).blur();
    });
    $('#common_mc_1').on('focus', function() {
        $('#common_mc_1 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
    });

    $('#common_mc_2 option:selected').html($('#common_mc_2 option:selected').attr('value')); // already changed onload

    $('#common_mc_2').on('change ', function() {
        $('#common_mc_2 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
        $('#common_mc_2 option:selected').html($('#common_mc_2 option:selected').attr('value'));
        $(this).blur();
    });
    $('#common_mc_2').on('focus', function() {
        $('#common_mc_2 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
    });

    $('#common_mc_3 option:selected').html($('#common_mc_3 option:selected').attr('value')); // already changed onload

    $('#common_mc_3').on('change ', function() {
        $('#common_mc_3 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
        $('#common_mc_3 option:selected').html($('#common_mc_3 option:selected').attr('value'));
        $(this).blur();
    });
    $('#common_mc_3').on('focus', function() {
        $('#common_mc_3 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
    });

    $('#common_mc_4 option:selected').html($('#common_mc_4 option:selected').attr('value')); // already changed onload

    $('#common_mc_4').on('change ', function() {
        $('#common_mc_4 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
        $('#common_mc_4 option:selected').html($('#common_mc_4 option:selected').attr('value'));
        $(this).blur();
    });
    $('#common_mc_4').on('focus', function() {
        $('#common_mc_4 option').each(function() {
            $(this).html($(this).attr('data-text'));
        });
    });
